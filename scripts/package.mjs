...
async function collect(directory, prefix = '') {
  for (const entry of (await readdir(directory, { withFileTypes: true })).sort((a, b) => a.name.localeCompare(b.name))) {
    ...
    if (entry.isDirectory()) await collect(new URL(entry.name + '/', directory), name + '/');
      ...
      if (!/^(?:icons\/(?:\d+\.png|icon\.svg)|(?:background|content|popup|loading|offscreen)\.js|(?:popup|loading|offscreen)\.html|(?:gate|ui|loading)\.css|themes\/[a-z]+\.(?:css|js)|manifest\.json|THIRD_PARTY_NOTICES\.txt)$/.test(name)) throw new Error(`Unexpected package file: ${name}`);
