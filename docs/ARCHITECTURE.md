...
7. A registered top-frame `document_start` content script covers the destination while it loads. It requests the profile immediately, decodes the model and waits for document readiness plus a short quiet period, capped at 200 ms; in theme mode, elements added after the reveal receive their roles for the next 20 seconds. It measures the live DOM locally. The matcher uses form names, labels, links, IDs, classes, descendant identities, and matched-parent relationships; changed article text can retain its component's historical styling.
...
`src/themes.ts` lists bundled per-site themes. Each is `static/themes/<id>.css` (rules written against `--n19-*` tokens, with light values on `html` and dark values on `html[data-net19-mode="dark"]`, plus the site's own design variables re-pointed at those tokens) and `static/themes/<id>.js`, which sets `globalThis.net19Theme`: how to read the site's own light/dark mode and, optionally, a map from the site's current palette colors to 2019 colors. The shared `palette.js` runs after it in the same isolated world: when `<body>` starts (stylesheets in `<head>` are parsed, nothing has painted), it marks `html[data-net19-mode]`, re-points every root custom property holding a mapped color, and repeats when the site changes mode. Palette maps are rescanned only on mode changes or new stylesheets (batched), with net19's own sheet disabled while the site's values are read, so the engine never wakes itself. The worker registers each theme as a top-frame `document_start` content script, re-registering only when the set of themes or paused sites changes; themed domains are excluded from the loading-page redirect and from the archive content script. Wikipedia additionally uses a declarativeNetRequest query transform (`useskin=vector`). Paused sites are excluded from all of these.

## Bounds

| Resource | Limit |
| --- | --- |
| Preflight wait | 8 seconds. Preparation continues in the background after release |
